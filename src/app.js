...
function start(type, preferredId) {
  ...
  mode = type; journey = makeJourney(preferredId, progress.rows, mode, progress.journeyOffset);
...
function advance() {
  ...
  if (station === journey.length - 1) {
    progress.trips++; progress.journeyOffset = nextJourneyOffset(progress.rows, progress.journeyOffset); save(); navigate('finish'); reward(); return;
  }
