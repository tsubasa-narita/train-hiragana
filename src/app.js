...
function chooseTrace(rows, letter) {
  traceState = newTrace(rows, letter, progress.recentImages);
  render(); speak(traceState.letter);
}
function startTrace() {
  traceState = newTrace(progress.rows, undefined, progress.recentImages);
  unlockRewardAudio(progress.sound); navigate('trace'); speak(traceState.letter);
}
function traceCompleted() {
  if (!progress.stamps.includes(traceState.card.id)) progress.stamps.push(traceState.card.id);
  progress.recentImages[traceState.letter] = traceState.card.image;
  save(); recordHistory(); chime(); speak('できたね！');
...
function prepare() {
  ...
  choices = makeChoices(letter, progress.level === 'match' ? 2 : 3, gameRows);
  progress.recentImages[letter] = journey[station].image;
  save();
}
function start(type, preferredId) {
  ...
  if (preferredId) {
    progress.rows = [kanaRow(TRAINS.find(t => t.id === preferredId).name[0])];
...
function reward(replay = false, restoring = false) {
  lastReward = showTrainReward({ train: replay ? lastReward : undefined, sound: progress.sound, speak, onDone: () => {
    ...
    if (rewardOpen) moveHistory(-1);
    else document.querySelector('.finish [data-action="home"]')?.focus({ preventScroll: true });
  } });
  if (!restoring) {
    rewardOpen = false; recordHistory();
    rewardOpen = true; history.pushState(snapshot(), '', '#reward');
  } else recordHistory();
}
...
function game() {
  ...
  return `<main class="game"><div class="game-top"><button class="quiet" data-action="home">← えきに もどる</button><span>${gameRows.length ? gameRows.join('・') + ' ぎょう' : 'ぜんぶの ぎょう'} / ${mode === 'find' ? 'もじを みつけよう' : 'なまえを つなごう'}</span><b>${station + 1} / 5 えき</b></div>
  ...
  <section class="game-board"><div class="train-picture"><img src="${imagePath(train)}" alt="${train.name}のイラスト"/>${train.historical ? '<span class="history-tag">むかしの てつどう</span>' : ''}<span class="picture-tag" style="--train-color:${train.color}">${train.name}</span></div>
...
function advance() {
  ...
  if (station === journey.length - 1) {
    progress.trips++;
    if (progress.rows.join() === gameRows.join()) progress.journeyOffset = nextJourneyOffset(gameRows, gameOffset); save(); navigate('finish', { replace: true }); reward(); return;
  }
...
function render() {
  ...
  if (screen === 'trace') disposeTrace = mountTrace(root, traceState, { change: recordHistory, complete: traceCompleted, speak, choose: chooseTrace, reset: () => { traceState = { ...traceState, stroke: 0, index: 0, complete: false }; render(); } });
}
function snapshot() {
...
function navigate(next, { replace = false } = {}) {
  cancelAdvance(); stopTrainReward(); stopVoice();
  const ancestor = trail.lastIndexOf(next);
  if (!replace && ancestor !== -1) {
    moveHistory(ancestor - trail.length);
    return;
  }
  if (!replace && next !== screen) {
    trail = [...trail, screen];
    history.pushState(null, '', '#' + next);
  }
  rewardOpen = false; screen = next; render(); window.scrollTo(0, 0);
}
...
function restoreHistory(state) {
  ...
  if (screen === 'game') {
    ...
    if (choices.length !== count) choices = makeChoices(journey[station].name[letterIndex], count, gameRows);
  }
  render(); window.scrollTo(0, 0);
  if (screen === 'game' && answered) advanceTimer = setTimeout(advance, 1000);
  if (screen === 'finish' && rewardOpen) reward(true, true);
}
...
root.addEventListener('click', e => {
  ...
  if (action === 'row') {
    ...
    progress.rows = normalizeRows(progress.rows.includes(row) ? progress.rows.filter(r => r !== row) : [...progress.rows, row]);
    progress.journeyOffset = 0;
    save(); render();
  }
...
if (history.state?.app === HISTORY_APP) restoreHistory(history.state);
else render();
